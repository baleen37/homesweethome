...
class TestCreateDongCodeParser:
    ...
    def test_creates_parser_with_dong_code_argument(self, mock_cli_args):
        ...
        mock_cli_args(["test", "--dong-code", "1150010100"])
        args = parser.parse_args()
        assert args.dong_code == ["1150010100"]

    def test_dong_code_argument_is_append_action(self, mock_cli_args):
        """--dong-code 인자는 여러 번 사용 가능"""
        parser = create_dong_code_parser("테스트 설명")

        mock_cli_args(["test", "--dong-code", "1150010100", "--dong-code", "1150010200"])
        args = parser.parse_args()
        assert args.dong_code == ["1150010100", "1150010200"]

    def test_dong_code_argument_defaults_to_none(self, mock_cli_args):
...
class TestAddAllArgument:
    ...
    def test_adds_all_flag_to_parser(self, mock_cli_args):
        """--all 플래그를 파서에 추가"""
        parser = create_dong_code_parser("테스트 설명")
        add_all_argument(parser)

        # --all 플래그가 있는지 확인
        mock_cli_args(["test", "--all"])
        args = parser.parse_args()
        assert args.all is True

    def test_all_flag_defaults_to_false(self, mock_cli_args):
        """--all 플래그 기본값은 False"""
        parser = create_dong_code_parser("테스트 설명")
        add_all_argument(parser)
...
class TestAddOutputArgument:
    ...
    def test_adds_output_argument_with_custom_default(self, mock_cli_args):
    ...
    def test_output_argument_can_be_overridden(self, mock_cli_args):
        """--output 인자 값 오버라이드"""
        parser = create_dong_code_parser("테스트 설명")
        add_output_argument(parser, default="default.csv", help="도움말")

        mock_cli_args(["test", "--output", "custom_output.csv"])
        args = parser.parse_args()
...
class TestResolveDongCodes:
    ...
    def test_returns_all_dong_codes_when_all_flag_is_true(self, mock_cli_args):
    ...
    def test_returns_provided_dong_codes_when_dong_code_argument_given(self, mock_cli_args):
    ...
    def test_returns_first_five_dong_codes_when_no_argument_provided(self, mock_cli_args):
    ...
    def test_ignores_all_flag_when_has_all_flag_is_false(self, mock_cli_args):
    ...
    def test_ignores_all_flag_when_has_all_flag_false_and_no_dong_code(self, mock_cli_args):
    ...
    def test_all_flag_takes_precedence_over_dong_code(self, mock_cli_args):
        ...
        mock_cli_args(["test", "--all", "--dong-code", "1150010100"])
        args = parser.parse_args()
        dong_codes = resolve_dong_codes(args, has_all_flag=True)
    ...
    def test_empty_dong_code_list_returns_default(self, mock_cli_args):
