...
본 보고서는 호갱노노(https://hogangnono.com) 웹사이트의 API 구조를 체계적으로 분석하여 시/도 > 구/군 > 동 > 아파트 단지 > 실거래 내역 순서로 데이터를 수집하는 방법을 정리한 문서입니다. MCP Playwright를 사용한 다단계 분석 과정을 통해 정확한 API 엔드포인트와 데이터 흐름을 파악했습니다.

**분석 기간**: 2025-01-09
**분석 방법**: Playwright MCP를 통한 실제 API 호출 및 네트워크 분석
...
호갱노노는 별도의 인증 없이 대부분의 데이터에 접근 가능한 RESTful API를 제공합니다.

**주요 특징**:
1. **별도 인증 불필요**: 대부분의 API는 세션 쿠키 없이 호출 가능
